...
struct Particle
{
    float3  worldPos; // World Space 기준 위치
    float   curTime; // 경과 시간
    float3  worldDir; // World Space 기준 어느 방향으로 이동하는 지
    float   lifeTime; // n초 동안 유지되어야 하는지
    int     alive; // 0이면 렌더링을 하지 않는다.
    float3  padding; // 16바이트 정렬을 위해 선언
};
...
// Compute Shader에서 공용으로 활용할 Buffer를 선언
RWStructuredBuffer<Particle> g_particle : register(u0);
RWStructuredBuffer<ComputeShared> g_shared : register(u1);

// CS_Main
// g_vec2_1 : DeltaTime / AccTime (Random Seed 값)
...
void CS_Main(int3 threadIndex : SV_DispatchThreadID)
{
    ...
    // 아주 정확한 값이 필요한 경우가 아닐 때에는 성능을 위해 동기화를 없애기도 한다.
    // 동기화를 위해 Barrier 사용
    ...
    if (g_particle[threadIndex.x].alive == 0)
    {
        ...
        while (true)
        {
            ...
            if (remaining <= 0) // 경쟁에서 패배
                break;

            int expected = remaining;
            int desired = remaining - 1;
            int originalValue;
            // 병렬 처리를 위해 Interlock 계열의 함수 사용
            // 무조건 한번만 실행된다.
            // addCount와 expected의 값이 같다면 addCount에 desired를 넣어준다.
            // originalValue애눈 addCount를 넣어준다.
