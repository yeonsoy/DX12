...
#pragma region TestObject
    ...
    vector<Vertex> vec(4);
    vec[0].pos = Vec3(-0.5f, 0.5f, 0.5f); // Vec3의 3번째(Z) 값이 1을 넘는다면 그려지지 않는다.
    ...
    transform->SetLocalScale(Vec3(120.f, 100.f, 1.f));

    shared_ptr<MeshRenderer> meshRenderer = make_shared<MeshRenderer>();
    {
        shared_ptr<Mesh> mesh = make_shared<Mesh>();
        mesh->Init(vec, indexVec); // 동일 모양의 Mesh는 한 번만 만들어서 재활용한다.
    ...
    }
    {
        ...
        texture->Init(L"..\\Resources\\Texture\\soybean.jpg");
