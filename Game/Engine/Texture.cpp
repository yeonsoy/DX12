...
void Texture::Load(const wstring& path)
{
    ...
    // UPLOAD Buffer를 만든다. (Constant Buffer에서도 활용한 부분)
    ...
    // RESOURCE_CMD_LIST : 리소스 로드 용도 Command List
	...
	D3D12_RESOURCE_FLAGS resFlags, Vec4 clearColor)
{
	...
	// Depth Stencil로 활용하는 Texture는 Render Target으로 동시에 활용 불가.
...
void Texture::CreateFromResource(ComPtr<ID3D12Resource> tex2D)
{
	...
	// - SRV (일반적인 Texture)
	// - RTV + SRV (별도의 Texture로 다른 Shader에 건넬 수도 있기 때문에 조합으로 활용)
